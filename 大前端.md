...
- Nodes版本>6.14，npm版本>3
- `npm install -D eslint`，然后 `npx eslint--init`
- 初始化配置文件基本概念：
  - env、extends、globals、rules等
...
- 在 eslint配置文件中配置 rules，对应不同类型的规则
- 在行內书写规则，需要写在`/*eslint…*/`

**ESLint配合IDE进行使用（VSCode&Webstorm）**

- VSCode需要下载插件+ ELInt库使用，Webster不用
- VSCode设置 Autofⅸx自动修复/Webstorm使用全文修复
- VSCode使用sync插件同步
...
//********************
//修改、更新数据 #region
User.findByIdAndUpdate('5c3732d30585fd1de0843b50', {
	password: '456'
}, function(err, ret){
	if(err){
		console.log('修改失败')
...
Redis与其他key-value缓存产品相比：支持数据的持久化，多数据结构list，set，zset，hash等的存储支持数据备份。
...
- 应用场景不一样：高性能缓存ⅴs海量数据分析

**docker 内安装**

参考 docker hub

- 手动方式安装（适合不熟悉 Docker/无 Docke环境）
- Docker方式安装（推荐）
- 配置 redis.conf，缓存 Fredis数据（生产需要）
...
- RediS gui具推荐：**[Another Redis DeskTop Manager](https://github.com/qishibo/AnotherRedisDesktopManager)**（免费）
- Medis（收费，可自构建）
...
Https（HyperText Transfer Protocol Secure）超文本传输安全协议，常称为 Http over Tls、Http over SSL或 Http Secure）是一种通过计算机网络进行安全通信的传输协议。
...
Redis 存储唯一验证码，设置时效性（auto expire）和UUID（key）。
...
- 设置短期的 Token有效，设置刷新 Token

#### 前端表单校验

**VSCode插件**：korofileheader ，生成注释。
...
**VSCode插件**：Node modules resolve 跳转到引入模块（和查看定义类似）
...
### 4. 掌握首页数据库设计、接口设计、文档更新
### 5. 首页列表、筛选、排序功能开发
### 6. 首页侧栏开发：统计、签到、周热议
### 7. 发帖编辑器自定义开发
### 8. 图片上传功能
### 9. 发帖功能：插入链接|图片、引用|代码样式、预览
### 10. MongDB多表联合查询及统计SQL
### 11. 发贴权限效验
